            ...
            let attr = if self.check(&token::Pound) {
                ...
                if attr_style != ast::AttrStyle::Outer {
                    ...
                    let mut err = self.sess.span_diagnostic.struct_span_err_with_code(
                        span,
                        DiagnosticMessage::Str(Cow::from("expected outer doc comment")),
                        error_code!(E0753),
                    );
                    if let Some(replacement_span) = self.annotate_following_item_if_applicable(
                        ...
                        err.note(DiagnosticMessage::Str(Cow::from("inner doc comments like this (starting with `//!` or `/*!`) can only appear before items")));
                        err.span_suggestion_verbose(
                            replacement_span,
                            DiagnosticMessage::Str(Cow::from("you might have meant to write a regular comment")),
                            "",
